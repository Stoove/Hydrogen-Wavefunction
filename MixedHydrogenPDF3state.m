...
% wavefunctions (given) where there is a phase factor between state 1 and
% the two other states (theta), and a phase factor between states 2 and 3
% (phi). The state amplitudes are 50-25-25.
%
...
if nargin > 2
    ...
    if nargin > 3
        ...
        if strcmp(plane,'xyz') % This simple function not designed to support xyz
            error(['MixedHydrogenPDF3state:: Does not support "xyz" input for '...
...
% Check that theta and phi are of size 1
if length(theta) ~= 1
    error('MixedHydrogenPDF3state:: Argument "theta" must be of size 1!')
end
if length(phi) ~= 1
    error('MixedHydrogenPDF3state:: Argument "phi" must be of size 1!')
end

coeffs = {1./sqrt(2)./sqrt(2),exp(-0.5i.*theta).*exp(1i.*phi)./2, ...
    exp(0.5i.*theta).*exp(1i.*phi)./2};
lims = [-8 8; -8 8];
nsams = [1000 1000];

if threeDflag % Implement different setings for the 3D case
    lims = [lims;-8 8];
    nsams = [200 200 200];
...
if (length(orbs) ~= 3) || ~isa(orbs,'cell')
    error(['MixedHydrogenPDF3state:: argument "orbs" must be cell array of '...
        'size 1x2 or 2x1.'])
end
