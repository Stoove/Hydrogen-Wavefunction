...
for i=1:sz(1)
    for j=1:sz(2)
        ...
        hold on
        [x,y,out] = MixedHydrogenPDF2state({'1s','2p-'},thets(i,j).*pi(),threeDcalc);
        %         surf(x,y,out,'LineStyle','none')
        if threeDcalc
            surf(x,y,out(:,:,101),'LineStyle','none')
%             surf(x,y,squeeze(out(:,101,:)),'LineStyle','none')
        else
            surf(x,y,out,'LineStyle','none')
        end
...
%         hold on
        xlim(xlims)
        ylim(ylims)
        title(['\theta = ' num2str(thets(i,j)) '\pi'])
        view(2)
